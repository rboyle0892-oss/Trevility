...
export default function OrganisationWorkspacePage() {
  ...
  async function importCsv(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setFilter('all'); setSearch(''); setShowAllActions(false); setPage(1);
  ...
  return (
    <main className="shell dashboard">
      ...
      <section className="empty"><div className="card"><div className="kicker">1 · Prepare</div><h2>Download the CSV template</h2><p>Use the blank template to keep contract, owner, SME, value and date fields consistent.</p><a className="button-secondary" download="trevecta-commercial-import-template.csv" href="/commercial-import-template.csv">Download CSV template</a></div><div className="card">{canManageCommercialData ? <><div className="kicker">2 · Upload</div><h2>Upload commercial data</h2><p>Trevecta validates the file before saving records.</p><label className="button-primary" style={{ display: 'inline-block', cursor: busy ? 'wait' : 'pointer' }}>{busy ? 'Importing…' : 'Choose CSV file'}<input accept=".csv,text/csv" disabled={busy} onChange={importCsv} style={{ display: 'none' }} type="file" /></label>{message && <div className="message success" role="status">{message}</div>}{warning && <div className="message error" role="alert"><strong>Records imported, readiness reconciliation failed.</strong><br />{warning}<br />Do not upload the file again.</div>}{error && <div className="message error" role="alert">{error}</div>}</> : <><div className="kicker">Read-only access</div><h2>Commercial imports are restricted</h2><p>You can review the register and BAU queue, but an owner, admin or member is required to import or change data.</p></>}</div></section>
